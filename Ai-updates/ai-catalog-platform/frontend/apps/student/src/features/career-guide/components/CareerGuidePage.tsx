...
import { useMyCareerGuides } from '../hooks/useCareerGuide';
import type { CareerGuideSummary } from '../types/career-guide.types';
...
function GuideCard({ guide }: { guide: CareerGuideSummary }) {
  return (
    <Link href={`/career-guide/${guide.id}`} className="group">
      <article className="h-full rounded-2xl border border-emerald-100 dark:border-emerald-500/15 bg-gradient-to-br from-emerald-50/70 via-white to-cyan-50/50 dark:from-emerald-500/[0.06] dark:via-white/[0.02] dark:to-cyan-500/[0.04] p-5 hover:shadow-lg hover:shadow-emerald-500/10 transition-all">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold text-gray-900 dark:text-white line-clamp-2 group-hover:text-emerald-600 dark:group-hover:text-emerald-400 transition-colors">
                {guide.title}
              </h3>
              <p className="text-[11px] text-gray-500 dark:text-slate-400">{guide.industry.icon} {guide.industry.name}</p>
            </div>
          </div>
          <span className={`text-[10px] px-2 py-1 rounded-full font-semibold ${DIFFICULTY_STYLE[guide.difficulty] || DIFFICULTY_STYLE.BEGINNER}`}>
            {guide.difficulty}
          </span>
          <SaveButton contentType="career-guide" contentId={guide.id} title={guide.title} />
        </div>

        <p className="text-sm text-gray-600 dark:text-slate-300 line-clamp-3 mb-3">{guide.description}</p>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-[11px] px-2 py-0.5 rounded-full bg-emerald-100/70 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300">
            {guide.toolsNeeded.length} tools
          </span>
        </div>
      </article>
    </Link>
...
export function CareerGuidePage() {
  const { industries, guides, loading } = useMyCareerGuides();
  ...
  if (industries.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-2">No Career Guides Yet</h2>
        <p className="text-sm text-gray-500 dark:text-slate-400 max-w-sm mx-auto">
          Career guides will appear here based on your selected industries.
        </p>
      </div>
    );
  }

  const filtered = activeTab === 'all' ? guides : guides.filter((g) => g.industrySlug === activeTab);
    ...
    <div className="space-y-6">
      <div className="rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 px-6 py-5 shadow-lg shadow-emerald-500/15">
        <div className="flex items-center gap-3">
          <Lightbulb size={20} className="text-white/90" />
          <div>
            <h1 className="text-lg font-extrabold text-white tracking-tight">Career Guide</h1>
            <p className="text-emerald-100 text-[11px] mt-0.5">Hands-on AI learning paths curated for your industries</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center gap-2 flex-wrap">
        ...
        {industries.map((ind) => {
          const count = guides.filter((g) => g.industrySlug === ind.slug).length;
          return (
            <button
              key={ind.slug}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-14">
          <p className="text-sm text-gray-400 dark:text-slate-500">No guides in this category yet.</p>
        </div>
      ) : (
