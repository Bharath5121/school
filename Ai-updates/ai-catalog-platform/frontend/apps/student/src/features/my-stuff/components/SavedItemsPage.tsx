...
import { myStuffApi, type SavedItem } from '../services/my-stuff.api';
...
const TYPE_META: Record<string, { label: string; emoji: string; color: string; pathFn: (item: SavedItem) => string }> = {
  discovery:      { label: 'Discovery', emoji: '🔬', color: 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300', pathFn: (i) => `/discoveries/${i.metadata?.slug || i.contentId}` },
  basic:          { label: 'Basics', emoji: '📚', color: 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300', pathFn: (i) => `/basics/${i.metadata?.slug || i.contentId}` },
  ...
  'lab-item':     { label: 'Lab', emoji: '🧪', color: 'bg-teal-100 dark:bg-teal-500/20 text-teal-700 dark:text-teal-300', pathFn: (i) => `/lab/${i.metadata?.categorySlug || 'item'}/${i.metadata?.slug || i.contentId}` },
  'trending-app': { label: 'App', emoji: '🚀', color: 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-300', pathFn: (i) => `/trending/${i.metadata?.slug || i.contentId}` },
};
...
export function SavedItemsPage() {
  ...
  useEffect(() => {
    myStuffApi.getSavedItems()
      .then(setItems)
      .catch(() => setItems([]))
      .finally(() => setLoading(false));
  }, []);

  const handleRemove = async (e: React.MouseEvent, contentType: string, contentId: string) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-2xl bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 px-6 py-5 shadow-lg shadow-emerald-500/15">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-extrabold text-white tracking-tight">Saved Items</h1>
            <p className="text-emerald-100 text-[11px] mt-0.5">Everything you&apos;ve bookmarked for later</p>
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-14">
          ...
          <h2 className="text-base font-bold text-gray-900 dark:text-white mb-1">No saved items yet</h2>
          <p className="text-xs text-gray-500 dark:text-slate-400 max-w-xs mx-auto mb-4">
            Save models, agents, apps, and guides from the explore pages.
          </p>
                  ...
                  <div className="relative space-y-2.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      <div className="ml-auto flex items-center gap-1">
                        <button
                          onClick={(e) => handleRemove(e, item.contentType, item.contentId)}
                          className="w-6 h-6 rounded-md flex items-center justify-center opacity-0 group-hover:opacity-100 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all"
                        >
                          <Trash2 size={12} />
                        </button>
                      </div>
