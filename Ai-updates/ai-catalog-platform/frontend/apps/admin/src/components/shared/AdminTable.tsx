...
function toRecord(item: object): Record<string, unknown> {
...
interface AdminTableProps<T extends object = object> {
  title: string;
  columns: Column<T>[];
  data: T[];
  loading: boolean;
  ...
  const filteredData = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      result = result.filter(item => {
        ...
        const val = r[searchKey] || r['title'] || r['name'] || '';
    ...
    for (const [key, value] of Object.entries(activeFilters)) {
      ...
      result = result.filter(item => {
        ...
        if (key === 'industrySlug') {
          const industry = r['industry'] as { slug?: string } | undefined;
          return r['industrySlug'] === value || industry?.slug === value;
        }
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredData.map((item, idx) => {
                  const r = toRecord(item);
                  return (
                    <tr key={String(r['id'] ?? idx)} className="border-b border-slate-200 dark:border-slate-700/20 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors">
                      {onReorder && (
                        <td className="px-3 py-3 text-center">
                          <input
                            type="number"
                            min={0}
                            defaultValue={(r['sortOrder'] as number | undefined) ?? idx}
                            onBlur={(e) => {
                              const val = parseInt(e.target.value, 10);
                              if (!isNaN(val) && val !== ((r['sortOrder'] as number | undefined) ?? idx)) {
                                onReorder(String(r['id']), val);
                              }
                            }}
                            onKeyDown={(e) => {
                              if (e.key === 'Enter') {
                      ...
                      {columns.map((col) => {
                        ...
                        return (
                          <td key={col.key} className="px-4 py-3 text-sm">
                            {col.render ? col.render(cellValue, item) : (
                              <span className="text-slate-600 dark:text-slate-300 truncate max-w-[200px] block">
                                {Array.isArray(cellValue) ? (cellValue as string[]).join(', ') : String(cellValue ?? '')}
                              </span>
                            )}
                          </td>
                        );
